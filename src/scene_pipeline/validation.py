...
def brief_checks(model,manifest,ir,*,robot_radius):
    ...
    for name,inst in manifest['instances'].items():
        ...
        generated_proxy=inst.get('source_classification',{}).get('allowed_use')=='static_collision'
        band=None if visual_only or generated_proxy else MASS_BANDS_KG.get(category)
        ...
        if generated_proxy:entry['mass_check_scope']='unverified_generated_static_collision_proxy'
